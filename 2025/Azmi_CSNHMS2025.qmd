...
dmsi_des <- svydesign(id = ~1, # cluster (PSU)
                      strata  = NULL, # define if applicable
                      weights = ~final_wt, # sampling weight (ADW × PSF)
                      data    = dmsi_ds_final)

svymean(~dm, design = dmsi_des) # weighted prevalence
mean(dmsi_ds_final$dm == "DM") # crude prevalence (unweighted)
